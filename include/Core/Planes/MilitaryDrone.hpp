...
namespace Core::Planes {
    ...
    class MilitaryDrone final : public IAirplane {
    public:
        explicit MilitaryDrone(const int id, const FlightRole role) :
            m_id(id), m_role(role),
            m_state(role == FlightRole::Arrival ? FlightState::FlyingIn :
                                                  FlightState::Parked) {}

        [[nodiscard]] int id() const override { return m_id; }
        [[nodiscard]] std::string type_name() const override { return "MilitaryDrone"; }
        [[nodiscard]] int required_runway_length() const override { return 40; }
        [[nodiscard]] float operation_time() const override { return 1.5f; }
        [[nodiscard]] int max_holding_circles() const override { return 1; }
